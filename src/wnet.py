...
class WNet(Network):
    ...
    def __init__(self, num_channels: int = 3, num_classes: int = 20, device_type: str = 'cpu'):
        r"""
        :param num_channels: Number of channels in the raw image data
        :param num_classes: Number of classes in the output class probabilities
        """
        super(WNet, self).__init__(device_type)
    ...
    def get_loss(self, labels: Tensor, inputs: Tensor) -> Tensor:
        ...
        labels, inputs = labels.to(device_type), inputs.to(device_type)
        ...
        ncut_loss = alpha * NCutLoss2D(device_type=device_type)(masks.to(self.device_type), inputs.to(self.device_type))
        mse_loss = beta * nn.MSELoss()(outputs, inputs.detach())
        smooth_loss = gamma * OpeningLoss2D(device_type=device_type)(masks)
